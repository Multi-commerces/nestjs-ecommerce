...
import { Product, ProductDocument, ProductLang, ProductLangDocument } from './data/product.schema';
import { ProductReadData, ProductSaveData, ProductMergeData } from './data/product-data';
...
export class ProductsService {
	...
	async findAll(): Promise<ProductReadData[]> {
		const documents = await this.productModel.find().populate('product-translations').exec();
	...
	async create(product: ProductSaveData): Promise<ProductReadData> {
		...
		for (const lang of langs) {
			...
			if (!savedProduct['translated-products']) savedProduct['translated-products'] = [];

			savedProduct['translated-products'].push(savedLang._id);
		}
		await savedProduct.save();

		return ProductMapper.toData(savedProduct.toObject());
	}
